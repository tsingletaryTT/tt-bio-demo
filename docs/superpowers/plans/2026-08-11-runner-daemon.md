...
These tests drive the **real** `Daemon` loop with a fake folder and a fake card
pool. That matters: this task's whole value is the wiring, and a suite that only
checked `main()`'s argument parsing and a dataclass's defaults would pass no
matter how badly the loop behaved. Every test below fails if the behavior it
names regresses — check that yourself as you write them.
...
def test_main_reports_preflight_failure_and_exits_non_zero(tmp_path, capsys):
    code = main([
...
Two behaviors above deserve a note, because they are the ones a plausible
implementation gets wrong:

- **The card claim happens in the loop, not in `_run_one`.** `_run_one` releases
  the card it was handed. That split exists so the `ValueError` race guard has
  somewhere to live; a test asserting `_run_one` emits a `busy` event would be
  pinning the wrong design.
- **A success resets the failure count.** Without that, a target that fails
  occasionally over a long booth day eventually accumulates three failures and
  disappears from the playlist, even though it works most of the time.

You must also write a test for the claim race itself — the loop catching
`ValueError` from `mark_busy`, requeueing the job, and carrying on. `_FakeCards`
above has `quarantine_on_claim` for exactly this. Driving the loop rather than
`_run_one` means bounding it; do that by stopping the daemon from inside a fake
