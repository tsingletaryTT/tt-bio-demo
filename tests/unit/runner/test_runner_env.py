...
# ── one visible chip: what tt-bio 0.6.3 requires of any in-process opener ───
#
# tt-bio 0.6.3 forces TT_MESH_GRAPH_DESC_PATH to a 1x1 P300 mesh-graph
# descriptor inside get_device() (tt_bio/tenstorrent.py, ensure_p300_mesh_
# descriptor). That is correct only when exactly ONE P300 chip is visible; with
# a whole board pair visible, the 1x1 descriptor disagrees with the cluster and
# ttnn refuses the open outright:
#
#   TT_FATAL @ tt_metal/fabric/control_plane.cpp:1262
#   Physical chip id 0 not found in control plane chip mapping.
#
# Production already satisfies this -- runner/workers.py pins each worker to
# one chip -- so these tests pin the rule itself rather than a shipped bug.

def test_a_multi_chip_visibility_string_is_narrowed_to_its_first_chip():
    from runner.env import single_visible_device
    assert single_visible_device("0000:01:00.0,0000:02:00.0") == "0000:01:00.0"


def test_a_single_chip_visibility_string_is_left_alone():
    from runner.env import single_visible_device
    assert single_visible_device("0000:03:00.0") == "0000:03:00.0"


def test_an_absent_or_empty_visibility_is_reported_as_none():
    """Unset means "every chip on the box is visible", which is exactly the
    multi-chip case this helper exists to prevent -- but the helper cannot
    invent a BDF it was never told. It returns None so the caller can decide
    (the integration conftest asks ttnn for the enumeration instead)."""
    from runner.env import single_visible_device
    assert single_visible_device(None) is None
    assert single_visible_device("") is None
    assert single_visible_device("  ,  ") is None
