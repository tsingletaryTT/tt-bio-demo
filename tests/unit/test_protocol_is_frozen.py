...
def test_the_protocol_version_is_four_on_this_side_too():
    ...
    3 -> 4 when affinity questions were added: one new client message
    (`question`) and three new events (`answer_start`, `answer_done`,
    `answer_error`), both directions changed, so both halves move."""
...
def test_the_client_vocabulary_is_exactly_three_messages():
    ...
    of its own", and that is exactly what the egg was, and now the question."""
